...
export function Dashboard() {
  ...
  const [icpPrice, setIcpPrice] = useState<number | null>(null); // ICP to USD
  const [loading, setLoading] = useState(true);
  // Fetch ICP price from CoinGecko (realtime)
  useEffect(() => {
    const fetchIcpPrice = async () => {
      try {
        ...
        setIcpPrice(6.2); // fallback
      }
    };
    fetchIcpPrice();
    const interval = setInterval(fetchIcpPrice, 60_000); // refresh tiap 1 menit
  ...
  const handleRefreshBalance = () => {
    if (window && typeof window !== "undefined") {
      // If ConnectPlug exposes a static method, call it here
      // Otherwise, trigger a re-fetch by remounting or using a ref
      // For now, simulate by setting loading and calling ConnectPlug again
      setLoading(true);
      setTimeout(() => {
        // This will trigger ConnectPlug to update balance
        // If ConnectPlug uses a callback, it will call setWalletBalance
        setLoading(false);
      }, 1000);
    }
  };
              ...
              <div className="flex items-center space-x-3">
                <div className="text-right">
                  <p className="text-2xl font-bold text-white">
                    {walletBalance !== null && icpPrice !== null
                      ? `$${(walletBalance * icpPrice).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USD`
                      : "Loading..."}
                  </p>
